...
function loadStatsPage(){
	var table = document.getElementById("stats_table");//Retrieve our table element
		...
		{//Outer for loop iterates over each row
				...
				opposingteam = table.rows[row_counter].cells[1].innerHTML;
				parseInt(cell_value1);
				parseInt(cell_value2);
				if (cell_value1 > cell_value2){
					winsval++;
					table.rows[row_counter].cells[4].innerHTML=cuboulder;
				}
				else {
					lossesval++;
			...
			document.getElementById("wins").innerHTML= winsval;
			document.getElementById("losses").innerHTML= lossesval;
		}
...
function switchPlayers(playerNum) {
	...
	document.getElementById("player_img").alt = curplayer.alt;
    ...
    var avgpass = Math.floor(curplayer.pass_yards / curplayer.games_played);
    var avgrush = Math.floor(curplayer.rushing_yards / curplayer.games_played);
    var avgrec = Math.floor(curplayer.receiving_yards / curplayer.games_played);
...
function loadPlayersPage() 
{
	...
	for(playercounter=0;playercounter<players.length;playercounter++) 
	{
		dropdownelements = dropdownelements +  "<a href='#' class = 'dropdown-item' onclick = 'switchPlayers("+ playercounter +")'>" + players[playercounter].name +"</a>"
	}
	playerselector.innerHTML = dropdownelements;
}
